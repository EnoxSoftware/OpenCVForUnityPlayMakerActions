...
namespace OpenCVForUnityPlayMakerActions
{
    ...
    public class Imgproc_getGaborKernel_1_CS : HutongGames.PlayMaker.FsmStateAction
    {
        ...
        void DoProcess ()
        {
            ...
            ((OpenCVForUnityPlayMakerActions.Mat)storeResult.Value).wrappedObject = OpenCVForUnity.Imgproc.getGaborKernel (new OpenCVForUnity.Size ((double)ksize_width.Value, (double)ksize_height.Value), (float)sigma.Value, (float)theta.Value, (float)lambd.Value, (float)gamma.Value, (float)psi.Value);
